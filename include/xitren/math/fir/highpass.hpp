...
class highpass : public filter<Type, Order + 1, TableType> {
...
public:
    ...
    prepare_table()
    {
        ...
        for (double& item : array) {
            ...
            item = (i == half ? 1.0 : 0.0)
                   - factor * filter_type::sinc(factor * (static_cast<double>(i++) - static_cast<double>(half)));
        }
        return array;
    }
     ...
     */
    highpass() : filter_type{prepare_table()} {}
    ...
    explicit highpass(std::array<double, N> const& data) : filter_type{prepare_table(), data}
    {}
